...
def load_model(modelUrl):
    if "." in modelUrl:
        return tf.keras.models.load_model(modelUrl)
    else:
        return tf.saved_model.load(modelUrl)


# memory = joblib.Memory(location='cache_directory')
memory = joblib.Memory(location='cached_models')

cached_load_model_v1 = memory.cache(tf.keras.models.load_model('my_model.h5'))
cached_load_model_v2 = memory.cache(tf.saved_model.load('model_v2'))
